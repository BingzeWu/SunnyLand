...
private:
    ...
    std::vector<std::pair<engine::object::GameObject*, engine::object::GameObject*>> collision_pairs_;/// @brief 检测并处理游戏对象和瓦片层之间的碰撞。
    std::vector<std::pair<engine::object::GameObject*, engine::component::TileType>> tile_trigger_events_; ///< @brief 记录游戏对象与瓦片类型的触发事件（如踩到陷阱瓦片）
...
public:
    ...
    const std::vector<std::pair<engine::object::GameObject*, engine::component::TileType>>& getTileTriggerEvents() const {return tile_trigger_events_;}; ///< @brief 获取当前的瓦片触发事件容器
...
private:
    void resolveTileCollisions(engine::component::PhysicsComponent* pc, float delta_time); /// @brief 处理可移动物体与SOLID物体的碰撞。
