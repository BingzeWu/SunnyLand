...
void UIInteractive::setSprite(const std::string &name)
{
    if (sprites_.find(name) != sprites_.end()) {
        current_sprite_ = sprites_[name].get(); // 设置当前显示的精灵 .get() 获取裸指针
    } else {
        spdlog::warn("UIInteractive::setSprite - 精灵 '{}' 不存在", name);
    }
}

void UIInteractive::addSound(const std::string &name, const std::string &path)
{
    sounds_[name] = path; // 直接存储路径，播放时再加载
}

void UIInteractive::playSound(const std::string &name)
{
    if (sounds_.find(name) != sounds_.end()) {
        context_.getAudioPlayer().playSound(sounds_[name]); // 通过路径播放音效
    } else {
        spdlog::warn("UIInteractive::playSound - 音效 '{}' 不存在", name);
    }
}
