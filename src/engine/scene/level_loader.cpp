    ...
    if (tile_json.contains("properties")) {
        ...
        for (auto& property : properties) {
            ...
            // 新增：检查 unisolid 属性
                ...
                if (slope_type == "1_0") return engine::component::TileType::SLOPE_1_0;
                if (slope_type == "0_2") return engine::component::TileType::SLOPE_0_2;
...
std::optional<nlohmann::json> LevelLoader::getTileJsonByGid(int gid) const
{
    ...
    for (const auto& tile_json : tiles_json) {
        ...
        if (tile_id == local_id) {   // 找到对应的瓦片，返回瓦片json
            return std::make_optional(tile_json);
        }
    }
    return std::nullopt;
}
